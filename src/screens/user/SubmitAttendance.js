...
function SubmitAttendance({ navigation, route }) {
  ...
  const handleCompare = async () => {
    try {
      const dbRefStyles = doc(db, 'attendances', makeid(20));
      setLoading(true);
      let imageBase64 = `data:image/jpeg;base64,${base64Data}`
  
      let data = {
        source_image: state.user.image,
        target_image: imageBase64
      }

      let res = await apiService.compareFace(data);

      if(res.data.Errors) {
        res.data.Errors.map((item) => {
          toast.show(item.Message + ', please make sure you take the image a well lit area');
        })
      }
      ...
      delete attendanceData.acceptingAttendance;

      // set user data in firestore
      let dataInfo = await setDoc(dbRefStyles, attendanceData);
      console.log(dataInfo);
      toast.show('Atendance submitted successfully');
      navigation.goBack();
      setLoading(false);
    } catch (error) {
      console.log(error)
    ...
    <SafeAreaView style={styles.main}>
      ...
      <View style={styles.header}>
        <View style={styles.userInfoContainer}>
          ...
          <Text style={[styles.name, { fontSize: wp(18) }]}>Snap your picture to submit your attendance</Text>
          <Text style={[styles.name, { fontSize: wp(18) }]}>to {details.courseTitle}</Text>
          <Text style={[styles.name, { fontSize: wp(18) }]}>by {details.lecturer.firstName} {details?.lecturer?.lastName}</Text>
        </View>
      </View>
      <View style={{ width: wp(335), height: wp(335), borderRadius: 9999, overflow: 'hidden', marginTop: hp(65), marginHorizontal: wp(20), }}>
        {!image && (
          <Camera
            // other props
            autoFocus={true}
      ...
      <View style={{ marginTop: hp(70), marginHorizontal: wp(20), }}>
